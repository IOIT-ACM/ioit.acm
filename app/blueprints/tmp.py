...
web3_interest_form_bp = Blueprint(
    "web3_interest_form",
    __name__,
    template_folder="../templates/tmp",
)

@web3_interest_form_bp.route("/web3-interest-form", methods=["GET", "POST"])
def interest_form():
    if request.method == "POST":
        ...
        if not API_URL or not BEARER_TOKEN:
            return make_response(
                jsonify({"error": "Server is not configured to handle this request."}), 500
            )
        ...
        try:
            sheetdb_response = requests.post(API_URL, json={"data": [data]}, headers=headers)
            sheetdb_response.raise_for_status()
            response = make_response(jsonify({"success": True, "message": "Data submitted successfully"}), 200)
        except requests.exceptions.RequestException as e:
            print("Error sending data to SheetDB: {}".format(e))
            response = make_response(
                jsonify({"error": "Failed to send data to the external service", "details": str(e)}), 502
            )
...
cloud_interest_form_bp = Blueprint(
    "cloud_interest_form",
    __name__,
    template_folder="../templates/tmp",
)

@cloud_interest_form_bp.route("/cloud-interest-form", methods=["GET", "POST"])
def interest_form():
    if request.method == "POST":
        origin = request.headers.get("Origin")
